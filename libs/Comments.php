...
class VOID_Widget_Comments_Archive extends Widget_Abstract_Comments
{
    ...
    private function threadedCommentsCallback()
    {
        ...
        if ($setting['VOIDPlugin']) {
            ...
            if (
                $metaArr['dislikes'] >= $setting['commentFoldThreshold'][0]
    ...
    <div class="comment-content-wrap">
        ...
        echo '</span>';
            ...
            <span>
                ...
                <!-- 根据IP获取地理位置 -->
                <a><?php echo convertip($this->ip); ?></a>
                ...
                <a style="margin: 0 5px" no-pjax target="_self" class="comment-vote vote-button"
                    href="javascript:void(0)" onclick="VOID_Vote.vote(this)" data-item-id="<?php echo $this->coid; ?>"
                    data-type="up" data-table="comment"><i class="voidicon-thumbs-up"></i> <span class="value">
                        <?php echo $metaArr['likes'] ?>
                    </span>
                </a>
                <a no-pjax target="_self" class="comment-vote vote-button" href="javascript:void(0)"
                    onclick="VOID_Vote.vote(this)" data-item-id="<?php echo $this->coid; ?>" data-type="down"
                    data-table="comment"><i class="voidicon-thumbs-down"></i> <span class="value">
                        <?php echo $metaArr['dislikes'] ?>
                    </span>
                </a>
                <?php } ?>
            </span>
        ...
        <div class="comment-content yue">
            <?php if (
            $setting['VOIDPlugin'] && $metaArr['dislikes'] >= $setting['commentFoldThreshold'][0]
            && ($metaArr['dislikes'] >= $metaArr['likes'] * $setting['commentFoldThreshold'][1])
        ) { ?>
            <span class="fold">[该评论已被大多数人点踩 | <a no-pjax target="_self" href="javascript:void(0)"
    ...
    public function pageNav($prev = '&laquo;', $next = '&raquo;', $splitPage = 3, $splitWord = '...', $template = '')
    {
        if ($this->options->commentsPageBreak && $this->_total > $this->options->commentsPageSize) {
            ...
            $nav = new Typecho_Widget_Helper_PageNavigator_Box(
                $this->_total,
                $this->_currentPage, $this->options->commentsPageSize,
                $query
            );
